...
# SAM1 imports
from segment_anything import sam_model_registry, SamAutomaticMaskGenerator as SamAutomaticMaskGenerator_SAM1, SamPredictor
...
class SegmentationStrategy:
    ...
    def segment_sel(self, sam, cvImage, maskType, selFile, boxCos, saveFileNoExt, formatBinary):
        raise NotImplementedError
...
class SAM1Strategy(SegmentationStrategy):
    ...
    def get_model_type_from_filename(self, model_filename):
        ...
        if model_type:
            ...
            print(f"Error: Could not auto-detect model type from SAM1 filename: {model_filename}")
            print(f"Please use one of the following file names: {list(self.MODEL_TYPE_LOOKUP.keys())}")
            return None
    ...
    def segment_box(self, sam, cvImage, maskType, boxCos, saveFileNoExt, formatBinary):
        ...
        masks, _, _ = predictor.predict(
            point_coords=None, point_labels=None, box=input_box, multimask_output=(maskType == 'Multiple')
        )
        saveMasks(masks, saveFileNoExt, formatBinary)
    ...
    def segment_sel(self, sam, cvImage, maskType, selFile, boxCos, saveFileNoExt, formatBinary):
        ...
        masks, _, _ = predictor.predict(
            point_coords=input_point, point_labels=input_label, box=input_box, multimask_output=(maskType == 'Multiple')
        )
        saveMasks(masks, saveFileNoExt, formatBinary)
    ...
    def run_test(self, sam):
        ...
        predictor.predict(point_coords=None, point_labels=None, box=input_box, multimask_output=False)
...
class SAM2Strategy(SegmentationStrategy):
    ...
    def get_model_type_from_filename(self, model_filename):
        ...
        if model_type:
            ...
            print(f"Error: Could not auto-detect model type from SAM2 filename: {model_filename}")
            print(f"Please use one of the following file names (or their .safetensors/.pt equivalents): {list(self.MODEL_TYPE_LOOKUP.keys())}")
            return None
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python seganybridge.py <model_type|auto> <checkpoint_path> [options]")
        return
    ...
    if model_filename.lower().startswith('sam_'):
        ...
        print(f"Error: Could not determine model family from filename: {model_filename}")
    ...
    try:
        if segType == 'Auto':
            ...
            strategy.segment_auto(sam, cvImage, saveFileNoExt, formatBinary, **auto_kwargs)
        elif segType in {'Selection', 'Box-Selection'}:
            selFile = sys.argv[8]
            boxCos = [float(val.strip()) for val in sys.argv[9].split(',')] if len(sys.argv) > 9 else None
            strategy.segment_sel(sam, cvImage, maskType, selFile, boxCos, saveFileNoExt, formatBinary)
        elif segType == 'Box':
            boxCos = [float(val.strip()) for val in sys.argv[8].split(',')]
            strategy.segment_box(sam, cvImage, maskType, boxCos, saveFileNoExt, formatBinary)
        else:
